...
live::Model::Model(LiveDevice& device, const std::vector<Vertex>& vertices) : liveDevice{ device } {
...
void live::Model::createVertexBuffers(const std::vector<Vertex>& vertices) {
	...
	void* data;
	vkMapMemory(liveDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
	memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
	vkUnmapMemory(liveDevice.device(), vertexBufferMemory);
}
